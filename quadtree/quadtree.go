...
func (qt *Quadtree) RetrieveByPoint(x, y float32) []Bounds {
	...
	if len(qt.Nodes) > 0 {
		...
		if index != -1 {
			...
			returnObjects = append(returnObjects, qt.Nodes[index].RetrieveByPoint(x, y)...)

		} else {

			//if pRect does not fit into a subnode, check it against all subnodes
			for i := 0; i < len(qt.Nodes); i++ {
				returnObjects = append(returnObjects, qt.Nodes[i].RetrieveByPoint(x, y)...)
			}
...
func (qt *Quadtree) RetrieveIntersections(x, y float32) []int32 {
	...
	for o := 0; o < len(potentials); o++ {
		if potentials[o].IntersectsByPoint(x, y) {
			foundIntersections = append(foundIntersections, potentials[o].ID)
		}
	}
